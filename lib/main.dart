  ...
  Future<List<Product>> fetchProducts() async {
    try {
      ...
      if (response.statusCode == 200) {
        // فك تشفير البيانات باستخدام UTF-8 لضمان ظهور الحروف العربية بشكل صحيح
        final decodedBody = utf8.decode(response.bodyBytes);
        debugPrint('Data received: $decodedBody'); // للطباعة وتصحيح الأخطاء
        // تحويل CSV إلى قائمة من المنتجات
        final rows = const CsvToListConverter().convert(decodedBody);
        return rows
            .skip(1) // تجاهل الصف الأول (Header) لتجنب ظهوره كمنتج
            // تنظيف البيانات: تجاهل الصفوف الفارغة والتأكد من وجود 4 أعمدة على الأقل
            .where((row) => row.isNotEmpty && row.length >= 4)
            // فلترة البيانات: التأكد من أن رابط الصورة موجود وصالح
            .where(
              (row) =>
...
class _HomePageState extends State<HomePage> {
  ...
  // دالة مساعدة لتحويل نص السعر (مثل "1,200") إلى رقم رياضي للترتيب
  double _parsePrice(String priceStr) {
    if (priceStr.isEmpty || priceStr == 'Price not found') return 0.0;
  ...
  void initState() {
    ...
    // تفعيل التحريك التلقائي للبنرات كل 4 ثوانٍ
  ...
  // دالة بناء البنرات الإعلانية الجذابة
  Widget _buildPromoBanners() {
    final banners = [
      {
        'title': 'خصومات تصل إلى 50%',
        'subtitle': 'على أدوات المطبخ العصرية',
      ...
      {
        'title': 'شحن مجاني',
        'subtitle': 'للطلبات أكثر من 1000 جنيه',
      ...
      {
        'title': 'منتجات جديدة',
        'subtitle': 'اكتشف أحدث صيحات تنظيم المنزل',
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: (banner['color2'] as Color).withValues(alpha: 0.4),
                  blurRadius: 8,
